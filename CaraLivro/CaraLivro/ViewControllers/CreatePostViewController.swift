...
final class CreatePostViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, Storyboarded {
    ...
    let addImagemButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(openImageOptions), for: .touchUpInside)
        return button
    }()
    ...
    func postButtonAction(attachmentType: String?, attachmentPath: String?) {
        if listType == .friends {
            ...
            let post = PostIntoGroupMural(id_poster: currentUserInUse?.idUserProfile ?? 0, visibility: publicFlag, text: inputText.text, attachment_type: attachmentType, attachment_path: attachmentPath)
